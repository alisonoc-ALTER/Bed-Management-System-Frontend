<!-- Bar chart in overview for hoispital with the ID no 4 -->
 <template>
...
export default {
    ...
    return {
      ...
      chartOptions: {
        ...
        plugins: {
          title: {
            display: true,
            text: "Hospital no 4" //HardCode or make dynamic
          }
        }
      },
      chartData: {
  ...
  methods: {
    async countDisabledBeds() {
      try {
        const response = await fetch('/api/beds/statuses?hospital_id=4'); // Adjust hospital_id
    ...
    async fetchHospitalName() {
      try {
        const response = await fetch('/api/hospitals/find?id=4'); 
        const data = await response.json();
