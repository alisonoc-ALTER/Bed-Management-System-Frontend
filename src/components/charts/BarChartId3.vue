<!-- Bar chart in overview for hoispital with the ID no 3 -->
 <template>
...
export default {
    ...
    return {
      ...
      chartOptions: {
        ...
        plugins: {
          title: {
            display: true,
            text: "Hospital no 3" //HardCode or make dynamic
          }
        }
      },
      chartData: {
 ...
 methods: {
    async countDisabledBeds() {
      try {
        const response = await fetch('/api/beds/statuses?hospital_id=3'); // Adjust hospital_id
    ...
    async fetchHospitalName() {
      try {
        const response = await fetch('/api/hospitals/find?id=3'); 
        const data = await response.json();
